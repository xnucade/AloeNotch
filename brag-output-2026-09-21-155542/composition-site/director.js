...
var ARRIVAL_CFG = { d: 0.34, bounce: 0.35 };     // Motion.arrival
...
var MODS = {
  media: $("mod-media"), timer: $("mod-timer"), clip: $("mod-clip"),
  shelf: $("mod-shelf"), cal: $("mod-cal")
};
var elSettings = $("settings"), elSTabs = $("stabs"), elWellEmpty = $("wellempty"),
    elCalDays = $("caldays"), elTogClip = $("stog-clip"), elHdr = document.querySelector(".hdr");
var CHIPS = [$("c0"), $("c1"), $("c2")];

// The week strip, centred on today.
(function () {
  var dows = ["S", "M", "T", "W", "T", "F", "S"], out = "";
  for (var i = -3; i <= 3; i++) {
    out += '<div class="calday' + (i === 0 ? " today" : "") + '">' +
           '<span class="d">' + dows[(i + 10) % 7] + "</span>" +
           '<span class="n rounded">' + (10 + i) + "</span></div>";
  }
  elCalDays.innerHTML = out;
})();

// Five tabs, matching the shipping SettingsView.
var STABS = [["\u2699", "General"], ["\u25A6", "Modules"], ["\u270E", "Appearance"],
             ["\u26BF", "Access"], ["\u24D8", "About"]];
function settingsTabs(active) {
  elSTabs.innerHTML = STABS.map(function (x, i) {
    return '<span class="stab' + (i === active ? " on" : "") + '">' +
           '<span class="si">' + x[0] + "</span>" + x[1] + "</span>";
  }).join("");
}
var stabsDrawn = -1;
...
var ICON = {
  speaker: '<svg width="30" height="30" viewBox="0 0 20 20" fill="currentColor"><path d="M9.4 3.2 5.6 6.4H2.8v7.2h2.8l3.8 3.2z"/><path d="M12.4 7a4 4 0 0 1 0 6" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round"/><path d="M14.8 4.6a7.2 7.2 0 0 1 0 10.8" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round"/></svg>',
  pods: '<svg width="28" height="28" viewBox="0 0 20 20" fill="currentColor"><path d="M6.4 2.2A2.8 2.8 0 0 0 3.6 5v4.2a2.8 2.8 0 0 0 5.6 0V5a2.8 2.8 0 0 0-2.8-2.8zM6.4 11.6a1.6 1.6 0 0 0-1.6 1.6v3.2a1.6 1.6 0 0 0 3.2 0v-3.2a1.6 1.6 0 0 0-1.6-1.6zM13.6 2.2A2.8 2.8 0 0 0 10.8 5v4.2a2.8 2.8 0 0 0 5.6 0V5a2.8 2.8 0 0 0-2.8-2.8zM13.6 11.6a1.6 1.6 0 0 0-1.6 1.6v3.2a1.6 1.6 0 0 0 3.2 0v-3.2a1.6 1.6 0 0 0-1.6-1.6z"/></svg>',
...
var T = {
  ...
  toShelf: 15.90,     // reshapes to the drop shelf
  drop: 16.55,        // three files land, one at a time
  toCal: 19.40,       // reshapes to the week
  toHud: 22.50,       // collapses to a volume readout
  hudEnd: 25.40,      // which gives way to a device announcement
  toOutput: 28.50,    // opens, and the header becomes a picker
  toSettings: 31.70,  // the notch leaves and the window arrives
  settingsTab: 33.50, // a tab switch
  settingsTog: 34.60, // and a toggle
  settingsOut: 36.10, // the window goes
  pullBack: 36.40,    // the camera retreats
  panelOut: 40.00     // the panel leaves; type only
};
...
function moduleAt(t) {
  ...
  // Reopened from the keyboard, and it has to open onto *something*.
  if (t < T.toShelf) return { key: "media", a: clamp01((t - T.keyOpen - 0.15) / 0.35) };
  if (t < T.toCal) return { key: "shelf", a: clamp01((t - T.toShelf) / 0.3) };
  if (t < T.toHud - 0.25) return { key: "cal", a: clamp01((t - T.toCal) / 0.3) };
  if (t < T.toOutput) return { key: "cal", a: clamp01(1 - (t - (T.toHud - 0.25)) / 0.25) };
  return { key: "media", a: clamp01((t - T.toOutput - 0.15) / 0.35) };
}
...
function opennessAt(t) {
  ...
  p -= springAt(t, T.toHud, COLLAPSE);              // down to a volume readout
  p += springAt(t, T.toOutput, EXPAND);             // open again for the picker
  p -= springAt(t, T.toSettings, COLLAPSE);         // out of the way of the window
  p += springAt(t, T.settingsOut, EXPAND);          // and back for the numbers
  p -= springAt(t, T.panelOut, COLLAPSE);           // closed for the last word
  return clamp01(p);
}

/** How wide the strip is when the panel is shut — the app grows "wings". */
var WING_WIDE = 84 * PT;                          // activityWingWidth(.wide)
function stripWidthAt(t) {
  ...
  // A level bar and a device name both ask for the wide wings.
  if (t >= T.toHud && t < T.toOutput) return NOTCH_W + WING_WIDE * 2;
  if (t >= T.toSettings && t < T.settingsOut) return NOTCH_W;           // out of the way
  if (t >= T.panelOut) return NOTCH_W;    // closing for the last word
...
function frame(t) {
  ...
  var scale = track(t, [[0, 2.55], [T.morph, 2.55], [T.morph + 0.95, 1.0],
                        ...
                        // The HUD and the device announcement are strip-sized:
                        // at rest scale they are a pill in an empty frame.
                        [T.toHud - 0.15, 1.0], [T.toHud + 0.35, 1.6], [T.toOutput - 0.25, 1.6],
                        [T.toOutput + 0.45, 1.0],
                        [T.pullBack, 1.0], [T.pullBack + 1.3, 0.74], [45, 0.74]]);
  var camY = track(t, [[0, 455], [T.morph, 448], [T.morph + 0.95, 150],
                       [T.toTimer - 0.15, 150], [T.toTimer + 0.32, 330], [T.timerOpen + 0.5, 150],
                       [T.toHud - 0.15, 150], [T.toHud + 0.35, 320], [T.toOutput - 0.25, 320],
                       [T.toOutput + 0.45, 150],
                       [T.pullBack, 150], [T.pullBack + 1.3, 196], [45, 196]]);
  ...
  if (p <= 0.3) {
    ...
    } else if (t >= T.toHud && t < T.hudEnd) {
      // Volume, climbing — the notch drawing what macOS would have drawn.
      var lvl = clamp01(0.28 + (t - T.toHud - 0.3) * 0.34);
      elL.innerHTML = '<span class="peekglyph">' + ICON.speaker + "</span>";
      elR.innerHTML = '<span class="hudbar"><b style="width:' + (lvl * 100).toFixed(1) + '%"></b></span>';
    } else if (t >= T.hudEnd && t < T.toOutput) {
      elL.innerHTML = '<span class="peekglyph">' + ICON.pods + "</span>";
      elR.innerHTML = '<span class="peekval">AirPods Pro</span>';
    } else {
  ...
  /* ---- Shelf: three files landing, one at a time ---------------------- */
  var anyChip = 0;
  for (var ci = 0; ci < 3; ci++) {
    var land = springAt(t, T.drop + ci * 0.42, ARRIVAL_CFG);
    anyChip = Math.max(anyChip, land);
    CHIPS[ci].style.opacity = clamp01(land * 1.4).toFixed(3);
    CHIPS[ci].style.transform = "scale(" + (0.6 + 0.4 * land).toFixed(3) + ")";
  }
  elWellEmpty.style.opacity = (1 - clamp01(anyChip * 2)).toFixed(3);

  /* ---- The header becomes a device picker ----------------------------- */
  var picking = t >= T.toOutput + 0.45 && t < T.toSettings - 0.3;
  elHdr.style.opacity = picking ? 0.18 : 1;
  var pick = $("pickrow");
  if (!pick) {
    pick = document.createElement("div");
    pick.id = "pickrow";
    pick.style.cssText = "position:absolute;left:40px;right:40px;top:74px;display:flex;gap:14px;align-items:center;opacity:0";
    pick.innerHTML = '<span class="gear">' + ICON.speaker + "</span>" +
      '<span class="wx" style="background:rgba(255,255,255,0.07)">MacBook Pro Speakers</span>' +
      '<span class="wx" id="pickon" style="background:rgba(71,153,255,0.22);color:#73bfff">AirPods Pro</span>';
    elExp.appendChild(pick);
  }
  pick.style.opacity = picking ? clamp01((t - T.toOutput - 0.45) / 0.3).toFixed(3) : 0;

  /* ---- The settings window -------------------------------------------- */
  var sIn = springAt(t, T.toSettings + 0.25, EXPAND) - springAt(t, T.settingsOut - 0.25, COLLAPSE);
  sIn = clamp01(sIn);
  elSettings.style.opacity = sIn.toFixed(3);
  elSettings.style.transform = "scale(" + (0.94 + 0.06 * sIn).toFixed(3) + ")";
  elSettings.style.transformOrigin = "50% 40%";
  var wantTab = t >= T.settingsTab ? 1 : 0;      // General → Modules
  if (wantTab !== stabsDrawn) { stabsDrawn = wantTab; settingsTabs(wantTab); }
  elTogClip.className = "stog" + (t >= T.settingsTog ? " on" : "");
...
line("#lb4", 16.6, 1.7);              // the shelf
line("#lb5", 20.1, 1.5);              // the week
line("#lb6", 23.2, 1.5);              // the HUD
line("#lb7", 26.2, 1.5);              // the device announcement
line("#lb8", 29.2, 1.5);              // sound output
line("#lb9", 33.4, 1.6);              // settings
...
// beat-grid, shifted into the longer cut's tail.
var NUM_BEATS = [37.75, 38.29, 38.84, 39.38];
["#n0", "#n1", "#n2", "#n3"].forEach(function (sel, i) {
  tl.fromTo(sel, { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.36, ease: "power2.out" }, NUM_BEATS[i]);
});
tl.to("#nums", { opacity: 0, duration: 0.45, ease: "power1.in" }, 40.05);

line("#price", 40.4, 1.05, 20);
tl.fromTo("#payoff", { opacity: 0, y: 18 }, { opacity: 1, y: 0, duration: 0.42, ease: "power3.out" }, 41.8);
tl.to("#payoff", { opacity: 0, duration: 0.5, ease: "power1.in" }, 44.3);
tl.fromTo("#mark", { opacity: 0, y: 22 }, { opacity: 1, y: 0, duration: 0.5, ease: "power2.out" }, 42.5);
tl.fromTo("#url", { opacity: 0 }, { opacity: 1, duration: 0.5, ease: "power2.out" }, 42.75);
tl.to(["#mark", "#url"], { opacity: 0, duration: 0.55, ease: "power1.in" }, 44.2);
